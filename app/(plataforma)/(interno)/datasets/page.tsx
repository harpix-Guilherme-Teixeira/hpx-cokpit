...
import { NovoConjunto } from "@/features/datasets/novo-conjunto";
...
export default async function PaginaDatasets() {
  ...
  const [{ data: conjuntos, error }, { data: paineis }] = await Promise.all([
    supabase
      .from("dad_conjunto")
      .select("id, chave, nome, descricao, grao, dono, cadencia, atualizado_em")
      .order("nome"),
            ...
            <ul className="divide-grey-300/50 divide-y">
              {conjuntos.map((c) => (
                <li key={c.id} className="px-5 py-4">
                  <Link href={`/datasets/${c.id}`} className="block">
                    <p className="text-grey-600 font-medium">{c.nome}</p>
                    <p className="text-grey-400 text-xs">
                      {c.grao === "medicao" ? "uma linha por período" : "uma linha por item"},
                      atualização {ROTULO_CADENCIA[c.cadencia as Cadencia] ?? c.cadencia}
                      {c.dono ? `, ${c.dono}` : ""}
                    </p>
                  </Link>
                </li>
